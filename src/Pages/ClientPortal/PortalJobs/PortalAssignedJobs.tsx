...
const PortalAssignedJobs = () => {
  const { data, isLoading, isError } = useClientJobs(EJobStatus.ASSIGNED);
  const [assignedJobs, setAssignedJobs] = useState<Job[]>([]);
  useEffect(() => {
    setAssignedJobs(
      data?.filter((job) => job?.jobStatus?.id == EJobStatus.ASSIGNED) ?? []
    );
  }, [data]);
  ...
  return (
    <>
      ...
      <Card mr={12} mb={5} p={10} borderRadius={8} boxShadow={"none"}>
        ...
        <Spacer></Spacer>
        <PaginationTable data={assignedJobs} table={table} />
      </Card>
    </>
  );
};

export default PortalAssignedJobs;
