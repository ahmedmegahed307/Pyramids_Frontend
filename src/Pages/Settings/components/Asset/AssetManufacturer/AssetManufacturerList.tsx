...
const columnHelper = createColumnHelper<AssetManufacturer>();
...
interface AssetManufacturerListProps {
  data: AssetManufacturer[];
  setDeleteAssetManufacturerId: (id: number) => void;
  setUpdateAssetManufacturer: (model: AssetManufacturer) => void;
  deleteModal: any;
  updateModal: any;
  createModal: any;
}

const AssetManufacturerList = ({
  data,
  setDeleteAssetManufacturerId,
  setUpdateAssetManufacturer,
  updateModal,
  deleteModal,
  createModal,
}: AssetManufacturerListProps) => {
  const pageTitleStore = usePageTitleStore();

  useEffect(() => {
    pageTitleStore.setPageTitle("Asset Manufacturers ");
  }, []);
              ...
              <Tr key={row.id} bg={index % 2 != 0 ? "Neutral.100" : "white"}>
                ...
                <Td>
                  <IconButton
                    ...
                    icon={<EditIcon />}
                    onClick={() => {
                      setUpdateAssetManufacturer(row.original);
                      updateModal.onOpen();
                    }}
                    variant={"outline"}
                    size={"sm"}
                  ...
                  <IconButton
                    ...
                    icon={<DeleteIcon />}
                    onClick={() => {
                      setDeleteAssetManufacturerId(row.original.id || 0);
                      deleteModal.onOpen();
                    }}
                    variant={"outline"}
                    size={"sm"}
...
export default AssetManufacturerList;
