...
export type UpdateAssetManufacturerValidation = z.infer<typeof schema>;
type UserFormProps = {
  onSubmit: (data: UpdateAssetManufacturerValidation) => void;
  defaultValue: AssetManufacturer;
};

const UpdateAssetManufacturer = ({ onSubmit, defaultValue }: UserFormProps) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  } = useForm<UpdateAssetManufacturerValidation>({
  ...
  const handleFormSubmit = (data: UpdateAssetManufacturerValidation) => {
    onSubmit(data);
    //reset();
  };
...
export default UpdateAssetManufacturer;
