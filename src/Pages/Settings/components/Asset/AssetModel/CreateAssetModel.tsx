...
export type CreateAssetModelValidation = z.infer<typeof schema>;
type UserFormProps = {
  onSubmit: (data: CreateAssetModelValidation) => void;
};

const CreateAssetModel = ({ onSubmit }: UserFormProps) => {
  const {
    register,
    handleSubmit,
    reset,
    formState: { errors, isSubmitting, isValid },
  } = useForm<CreateAssetModelValidation>({
    resolver: zodResolver(schema),
  });
  const handleFormSubmit = (data: CreateAssetModelValidation) => {
    onSubmit(data);
    //reset();
  };
